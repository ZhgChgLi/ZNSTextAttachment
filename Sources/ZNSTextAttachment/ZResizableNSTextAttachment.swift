...
/// The replacement attachment produced once a `ZNSTextAttachment` finishes
/// loading. Sized according to the originally-requested fixed width / height,
/// or scaled to fit the line fragment when neither is set.
public class ZResizableNSTextAttachment: NSTextAttachment {

    /// Original size of the loaded image, if it could be decoded.
    public let imageSize: CGSize?

    /// Width override supplied at creation time.
    public let fixedWidth: CGFloat?

    /// Height override supplied at creation time.
    ...
    public override func attachmentBounds(for textContainer: NSTextContainer?, proposedLineFragment lineFrag: CGRect, glyphPosition position: CGPoint, characterIndex charIndex: Int) -> CGRect {
        if let fixedWidth = self.fixedWidth, let fixedHeight = self.fixedHeight {
            return CGRect(origin: .zero, size: CGSize(width: fixedWidth, height: fixedHeight))
        }

        guard let imageWidth = imageSize?.width, let imageHeight = imageSize?.height else {
            return .zero
        }
